%% PicoScope 9200 Series Sampling Oscilloscopes Example Code for communicating with the PicoScope 9200 Application using ActiveX.
%  
% Description:
%       Demonstrates how to call commands using the PicoScope 9200 application 
%       and ActiveX.
...
function varargout = PicoScope_9200_Example(varargin)
    ...
    gui_State = struct('gui_Name',       mfilename, ...
                       ...
                       'gui_OpeningFcn', @PicoScope_9200_Example_OpeningFcn, ...
                       'gui_OutputFcn',  @PicoScope_9200_Example_OutputFcn, ...
                       'gui_LayoutFcn',  [], ...
                       'gui_Callback',   []);
...
% --- Executes just before PicoScope_9200_Example is made visible.
function PicoScope_9200_Example_OpeningFcn(hObject, eventdata, handles, varargin)
    
    % Choose default command line output for PicoScope_9200_Example
    handles.output = hObject;

    % Makes the buttons only useful if PicoScope 9200 is open
    ...
    % UIWAIT makes PicoScope_9200_Example wait for user response (see UIRESUME)
    % uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = PicoScope_9200_Example_OutputFcn(hObject, eventdata, handles)
...
function OnButton_Callback(hObject, eventdata, handles)
    ...
    % Creates object to hold PicoScope 9200 
    handles.h = actxserver('PicoScope9000.COMRC');
...
function Command_Callback(hObject, eventdata, handles)
    ...
    % Do nothing here as we do not need to check when text has been changed
...
function StopButton_Callback(hObject, eventdata, handles)
    ...
    % Do not need guidata(hObject, handles) because not changing stored variable
    handles.h.ExecCommand('*StopSingle Stop');
...
function SingleButton_Callback(hObject, eventdata, handles)
    ...
    % Do not need guidata(hObject, handles) because not changing stored variable
    handles.h.ExecCommand('*StopSingle Single');
